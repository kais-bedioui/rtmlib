...
**⚠️ Correction (found while building `RTMO_VS_YOLO26S_POSE.md`): this "45 ms/frame" was very likely measured on GPU, not CPU.** `benchmark_recall.py`'s `yolo(...)` calls never pass `device=`, and Ultralytics' `YOLO()` auto-selects CUDA when available — this machine has an NVIDIA RTX 500 Ada dGPU. A direct spot-check confirms it: `yolo26x.pt` on this same clip measures **28.5 ms/frame on GPU** (device=0, matching the ballpark of the original 45 ms number) vs. **419.2 ms/frame forced onto CPU** (device='cpu') — over 9× slower, and now consistent with (slower than) RTMO-lightweight's CPU numbers rather than beating them. The "reversal" above isn't RTMO-lightweight being unexpectedly slow — it's an undetected GPU-vs-CPU mismatch. Treat the 45 ms figure as YOLO26x-on-GPU, not YOLO26x-on-CPU, and disregard the "reversal" framing. (The recall/false-positive numbers elsewhere in this document are unaffected — device choice changes speed, not detections.)
