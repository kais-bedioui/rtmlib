...
- **A `backend='tensorrt'` option for rtmlib — the capability exists and is verified on two machines, but rtmlib's own dispatch integration is still unstarted.** A separate worktree in this same checkout (`worktree-rtmo-tensorrt`, pushed to `origin/worktree-rtmo-tensorrt`) adds `RTMOTensorRT` (`rtmlib/tools/pose_estimation/rtmo_tensorrt.py`): RTMO running directly on a TensorRT engine (subclasses `RTMO`, overrides only `inference()`, reuses all of RTMO's SimCC/NMS decode unchanged) plus an FP16 engine builder. Verified on *this laptop's* RTX 500 Ada (`rtmo-m`/"balanced": 74.5 fps, beating this report's own onnxruntime/CUDA number for the same tier — §4: 48.9 fps — by ~1.5×) **and, in a later follow-up session, on a Jetson AGX Orin** (§11.3: 48.5-83.4 fps end-to-end, 11-20× faster than onnxruntime/CPU on the same hardware) — genuinely cross-platform now, not an x86-only prototype. What's still missing is packaging: it isn't merged into `benchmark/pipelines.py`/`run_bench.py`, so neither machine's numbers are in §4's table on equal footing with the other backends, and rtmlib itself has no `backend='tensorrt'` string a caller could pass to the existing classes — using it today means importing `RTMOTensorRT` directly.
...
### 11.3 Follow-up session: `RTMOTensorRT` verified end-to-end on this Jetson — closes the §11.1 gap

§11.1 flagged a specific gap: `trtexec`'s numbers are bare-engine only (no pre/post-processing, no Python), and a "directly-comparable, end-to-end FPS number... would need a small Python/TensorRT runner reusing rtmlib's pre/post-processing." That runner now exists — `RTMOTensorRT` (`rtmlib/tools/pose_estimation/rtmo_tensorrt.py`, built and x86-verified in the sibling `worktree-rtmo-tensorrt`, §9) — and this follow-up session ran it **for the first time on a Jetson**, not just on x86. This also directly answers a standing question about this project: is TensorRT-based deployment actually viable across the Jetson device family, not just architecturally plausible.

**Setup** (device reachable again at `relai-orin` after being offline for a stretch of this project): root disk was still tight (2.2 GB free of 54 GB), so everything — the `rtmlib` package copy, `cuda-python`, `onnxruntime` (CPU, for a same-run baseline), engine cache, `TMPDIR`, `pip` cache — was routed to the separate 3.6 TB NVMe at `~/Data/human_activity_recognition/rtmlib/` (2.7 TB free there) via `--target=`/`TMPDIR`/`TORCH_HOME`, keeping root untouched throughout. `tensorrt` (10.16.2.10) and `numpy`/`cv2` were already present system-wide; only `cuda-python` needed installing, and it's a genuine native aarch64 wheel (`cuda_bindings-13.4.3-cp312-cp312-manylinux_2_28_aarch64`) — the Tegra/Python-API-parity risk §9 flagged as unverified didn't materialize.

**Methodology**: reused the exact same clip §11.2's rfdetr-pose numbers were measured against (`~/Data/human_activity_recognition/rfdetr-pose/SHORTER_GX017153_W001_0_720p.MP4`), so this table is directly comparable to that one — same hardware, same video, same 720p resolution, unlike §11.1's engine-only numbers. `rtmo-m` ("balanced" tier), 40 timed frames + 5 warmup, same convention as §3/§4.

| Backend | Mean latency | FPS | vs. onnxruntime/CPU |
|---|--:|--:|--:|
| onnxruntime / CPU | 235.3 ms | 4.25 | 1× |
| **RTMOTensorRT FP32** | 20.61 ms | **48.52** | **11.4×** |
| **RTMOTensorRT FP16** | 11.99 ms | **83.35** | **19.6×** |

Correctness: both precisions detect the same person on a fixed test frame (COCO-17, mean keypoint score 0.751, identical between FP32/FP16) and match onnxruntime's `avg_persons` on the timed clip exactly (1.925 vs. 1.93) — no missed or spurious detections from FP16 quantization. Build times (FP32 134.8s, FP16 410.5s) closely match §11.1's `trtexec` build times for the same model (128.6s / ~7 min) — the Python builder API isn't meaningfully slower to build with than the CLI. FP16's 1.72× speedup over FP32 here matches the 1.75× measured on the x86 laptop (§9) — consistent across architectures.

**This closes the loop on §11's whole thread, decisively**: onnxruntime's CUDA EP is confirmed a hard dead end on this hardware (missing SM 8.7 kernels, this section's opening finding) — not merely slower than TensorRT, entirely non-functional. onnxruntime/CPU works but is far too slow for real-time (4.25 fps). **TensorRT — specifically through `RTMOTensorRT`, now verified as more than an x86-only prototype — is not just the best GPU option on Jetson, it's the only viable one, and it delivers genuinely real-time throughput (48-83 fps) once used.** `rtmlib` itself still has no `backend='tensorrt'` dispatch option (§9) — that packaging work remains open — but the underlying capability is now proven across both machines this project has tested: an x86 discrete GPU and an embedded Jetson SoC.
...
### 13.2 A static high-confidence false positive from yolox-tiny, and the fix

The first recording attempt (leaning over a desk, close to camera — a harder, self-occluding pose) showed a visibly wrong second skeleton in the background alongside the real one. Investigating live against the camera found it wasn't pose noise at all: `yolox-tiny` was firing a **completely static false-positive person detection** at pixel-identical coordinates (`[951, 292, 996, 391]` ± 1px, a 45×99px box) on a piece of background lab equipment, every single frame, confirmed across two separate recording sessions. Sweeping `score_thr` from its default (0.7) up to 0.95 **did not remove it** — the false positive's own confidence exceeds 0.95, so no amount of threshold tuning fixes it without also risking real detections in poorer lighting.

The fix instead exploits geometry, not confidence: a 45×99px box is only ~14% of the 720px frame height, far smaller than a real person standing at normal webcam distance. `benchmark/realsense_record_wholebody.py` now applies a **minimum bounding-box-height filter** (`--min-height-frac`, default 0.2 = 20% of frame height) to `yolox-tiny`'s output before it reaches the pose model — a one-line post-filter, not a retrain or a per-scene special case, and a filter that's generally good practice for real-time human-pose pipelines regardless of this specific false positive (it also skips wasting pose-model compute on spurious tiny boxes). Re-recorded standing upright after this fix: exactly 1 person detected throughout, no ghost.

This is worth flagging as a real limitation of the `yolox-tiny` tier specifically (the smallest/cheapest detector in rtmlib's zoo, chosen throughout this report for its speed) rather than of `dwpose-t` or the body+hands slicing — a larger detector (`yolox-m`/`yolox-x`, §1/§4) would very plausibly not have hallucinated on this background object in the first place, at some FPS cost.

### 13.3 Recorded demo video

`benchmark/realsense_demo/realsense_bodyhands_openvino_cpu.mp4` (18 MB, 1280×720, 449 frames / 14.9s, recorded standing upright, background-false-positive filter active per §13.2) — a genuine live capture-and-infer loop (not a replay of pre-recorded frames): each frame is grabbed from the camera, run through the full detector+filter+pose+slice+overlay pipeline, and only *then* is the next frame grabbed, so playback speed is an honest measurement, not a fixed guess. Encoded at the **achieved** rate (30.0 fps) rather than a nominal one. Per-frame live FPS, backend, and detected person count are burned into the top-left corner of every frame.

**~30 fps achieved is camera-limited, not model-limited**: §13's isolated probe measured the OpenVINO/CPU pipeline itself at 33.4 fps, faster than the D435I's native 30 fps color stream — `wait_for_frames()` can't return frames faster than the camera produces them, so end-to-end the pipeline is waiting on the camera, not the other way around. In other words, this pipeline has FPS headroom to spare at 1280×720 on CPU alone.
